...
try:
    ...
    print("Please run: pip install python-magic-win64 (on Windows) or pip install python-magic (on Linux/macOS)", file=sys.stderr)
    sys.exit(1)
...
def perform_signature_analysis(runner: VolatilityPluginRunner, mem_file_path: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not filescan_results:
            print("[!] No files found by filescan. This could indicate a profile mismatch. Skipping file analysis.")
        else:
            total_files_found = len(filescan_results)
            print(f"[*] Found {total_files_found} total open files.")
            checkable_files = [f for f in filescan_results if os.path.splitext(f.get("Name", ""))[1].lower() in EXT_TO_SIGNATURE]
            files_checked_count = len(checkable_files)
            print(f"[*] Found {files_checked_count} open files with checkable extensions.")
            ...
            for i, file_obj in enumerate(checkable_files):
                ...
                clear_line = "\x1b[K"
                progress_text = f"  -> Checking [{i+1}/{files_checked_count}]: {file_name}"
                print(f"\r{progress_text}{clear_line}", end="")
                # --------------------------------

                try:
                    dump_cmd = ["vol", "-q", "-f", mem_file_path, "windows.dumpfiles", f"--virtaddr={virt_addr}", f"--dump-dir={temp_dir}"]
        ...
        if not pslist_results:
            print("[!] No processes found by pslist. This strongly suggests a profile mismatch. Skipping process analysis.")
        else:
            total_procs_found = len(pslist_results)
            print(f"[*] Found {total_procs_found} total running processes.")
            checkable_procs = [p for p in pslist_results if os.path.splitext(p.get("ImageFileName", ""))[1].lower() in EXT_TO_SIGNATURE]
            procs_checked_count = len(checkable_procs)
            print(f"[*] Found {procs_checked_count} processes with checkable extensions.")
            ...
            for i, process in enumerate(checkable_procs):
                ...
                try:
                    dump_cmd = ["vol", "-q", "-f", mem_file_path, "windows.procdump", f"--pid={pid}", f"--dump-dir={temp_dir}"]
                    subprocess.run(dump_cmd, check=True, capture_output=True)
...
def print_report(results: dict, files_checked: int, procs_checked: int):
    ...
    print(f"  -> Summary: {suspicious_files_count}/{files_checked} suspicious files found.")
    ...
    if suspicious_procs_count > 0:
        ...
        for item in results['processes']:
            print(f"  - Process:  {item['filename']} (PID: {item['pid']})\n    Expected: {item['expected']}\n    Actual:   {item['actual']}")
    else:
        print("\n[*] No process signature mismatches were detected.")
    
    print(f"  -> Summary: {suspicious_procs_count}/{procs_checked} suspicious processes found.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Signature Analysis tool for memory dumps. Uses volatility_runner.py.")
    parser.add_argument("-f", "--file", required=True, help="Path to the raw memory dump file (.raw, .vmem, etc.)")
    ...
    analysis_results, files_total, procs_total = perform_signature_analysis(vol_runner, args.file)
